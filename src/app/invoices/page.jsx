...
const dropdownInputCls = `w-full px-3.5 py-2.5 rounded-xl text-sm font-medium
  bg-white dark:bg-[#1a1714] border border-gray-200 dark:border-white/[0.07]
  text-gray-900 dark:text-white
  focus:outline-none focus:border-indigo-500 dark:focus:border-[#e8b84b]
  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className={`${dropdownInputCls} flex items-center justify-between gap-2 text-left ${!displayLabel ? 'text-gray-400 dark:text-gray-600' : ''}`}
      >
...
function UploadModal({ projects, onSuccess, onCancel }) {
  ...
  const projectOptions = projects.map(p => ({
    value: p._id,
    label: p.title + (p.client ? ` — ${p.client.name}` : ''),
  }));

  const milestoneOptions = [
    { value: '', label: 'Project-level invoice' },
    ...milestones.map(m => ({ value: m._id, label: `${m.title} — ${m.percentage}%` })),
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      {!file ? (
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all
            ${drag
              ...
              : 'border-gray-200/80 dark:border-white/[0.07] hover:border-indigo-300 dark:hover:border-indigo-600 hover:bg-gray-50 dark:hover:bg-white/[0.02]'
            }`}
        >
          <Upload size={22} className="mx-auto mb-2 text-gray-400" />
          <p className="font-display font-semibold text-[13px] text-gray-700 dark:text-gray-300">
            Drop invoice here or <span className="text-indigo-500">browse</span>
          </p>
          <p className="font-display text-[12px] text-gray-400 mt-0.5">PDF, JPG, PNG — max 10MB</p>
        ...
        <div className="flex items-center gap-3 p-4 rounded-xl border"
          ...
          <div className="flex-1 min-w-0">
            <p className="font-display font-semibold text-[13px] text-gray-800 dark:text-white truncate">{file.name}</p>
            <p className="font-display text-[12px] text-gray-400">{fmtSize(file.size)}</p>
          </div>
          <button onClick={() => setFile(null)}
            className="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors cursor-pointer border-none bg-transparent">
            <X size={14} />
          </button>
        </div>
      )}
      ...
      {/* Project */}
      <div>
        <label className="block text-[11px] font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 mb-1.5">Project *</label>
        <CustomSelect
          value={projectId}
      ...
      {milestones.length > 0 && (
        <div>
          <label className="block text-[11px] font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 mb-1.5">Link to Milestone (optional)</label>
          <CustomSelect
            value={milestoneId}
            onChange={setMilestoneId}
            options={milestoneOptions}
            placeholder="Project-level invoice"
          />
        </div>
      )}
      ...
      {/* Label */}
      <div>
        <label className="block text-[11px] font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 mb-1.5">Label (optional)</label>
      ...
      <div className="flex gap-3 justify-end pt-1">
        <button onClick={onCancel} disabled={uploading}
          className="px-5 py-2.5 rounded-xl text-sm font-semibold bg-gray-100 dark:bg-white/[0.06] text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-white/[0.07] cursor-pointer hover:border-indigo-400 transition-all">
          Cancel
        </button>
        <button onClick={handleUpload} disabled={!file || !projectId || uploading}
          className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold cursor-pointer disabled:opacity-50 transition-all bg-indigo-500 dark:bg-[#e8b84b] text-white dark:text-black border-none">
    ...
    try {
      ...
      results.forEach((r, i) => {
        if (r.status === 'fulfilled') {
          ...
          invs.forEach(inv => all.push({ ...inv, _project: projs[i] }));
        }
      });
      ...
      <PageHeader
        ...
        action={
          <button
            ...
            className="bg-indigo-500 dark:bg-[#e8b84b] text-white dark:text-black font-semibold inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm cursor-pointer border-none"
          >
        ...
        <div className="relative flex-1">
          ...
          <input
            className="w-full pl-9 pr-4 py-2.5 rounded-xl text-sm font-medium
              ...
              focus:outline-none focus:border-indigo-500 dark:focus:border-[#e8b84b]
              focus:ring-2 focus:ring-indigo-500/20 transition-all"
            placeholder="Search by label, project, client…"
            value={search}
          ...
          <EmptyState
            ...
            action={
              <button onClick={() => setShowUpload(true)}
                className="bg-indigo-500 dark:bg-[#e8b84b] text-white dark:text-black font-semibold inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm cursor-pointer border-none">
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse" style={{ minWidth: 780 }}>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {filtered.map(inv => (
                  <tr key={inv._id} className="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="font-display font-bold text-[14px] text-gray-900 dark:text-white leading-none truncate max-w-[180px]">
                            {inv.label || inv.fileName}
                          </p>
                          {inv.label && (
                            <p className="font-display text-[11px] text-gray-400 dark:text-gray-500 mt-1 truncate max-w-[180px]">{inv.fileName}</p>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    {/* Project */}
                    <td className="px-5 py-4">
                      <span className="flex items-center gap-1.5 font-display font-semibold text-[13px] text-gray-600 dark:text-gray-300">
                        <FolderKanban size={12} className="text-gray-400 flex-shrink-0" />
                        {inv._project?.title || '—'}
                      </span>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      {inv._project?.client ? (
                        <span className="flex items-center gap-1.5 font-display font-semibold text-[13px] text-amber-600 dark:text-amber-400">
                    ...
                    <td className="px-5 py-4">
                      ...
                      ) : <span className="font-display text-[12px] text-gray-400 dark:text-gray-500">Project-level</span>}
                    </td>

                    {/* Date */}
                    <td className="px-5 py-4">
                      <span className="font-display text-[13px] text-gray-500 dark:text-gray-400">{fmtDate(inv.createdAt)}</span>
                    </td>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          className="w-8 h-8 rounded-lg flex items-center justify-center
                            bg-gray-100 dark:bg-white/[0.06]
                            border border-gray-200 dark:border-white/[0.07]
                            text-red-400 hover:border-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all cursor-pointer"
                          title="Delete"
                        >
                          <Trash2 size={13} />
                        </button>
